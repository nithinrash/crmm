...
router.put('/update', async (req, res) => {
  ...
  try {
    ...
    const [updateResult] = await connection.execute(updateSql, values);
    ...
    const auditSql = `
      INSERT INTO audit (
        ...
        Received_Lost, Comment, Status, AssignedTo, appointment_date,
        appointment_time, customer_appoinemnt, Authontic, after_sales, 
        reminder
      )
      VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)
    `;
    ...
    const auditValues = [
      ...
      sanitizedData.BDM_REMARKS_AFTER_MEET || 'No Remarks', // Replace NULL with default value
      sanitizedData.Follow_Up || '',
      sanitizedData.Feedback || '',
      sanitizedData.Quotation_Value || '',
      ...
      sanitizedData.reminder || ''
    ];

    // Log sanitized values for debugging
    console.log('Sanitized Audit Insert Values:', auditValues);
